...
class ICON(pl.LightningModule):
    ...
    #creato da me per vedere che cazzo succede con le immagini
    def save_image_test(self, image, nome):
        # image_test = resize(
        #         ((image + 1.0) / 2.0 * 255.0).transpose(1, 2, 0),
        #         (512, 512),
        #         anti_aliasing=True,
        #     )
        test_path = f"./data/images/{self.cfg.name}/test/{nome}.png"
        os.makedirs(os.path.dirname(test_path), exist_ok=True)
        test_pil = Image.fromarray(image.astype('uint8'))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Log memory usage
        log_memory_interval = 200
        if batch_idx % log_memory_interval == 0:
            print(f"Batch {batch_idx}, Memory Allocated: {torch.cuda.memory_allocated() / 1024**2:.2f} MB, "
                f"Memory Reserved: {torch.cuda.memory_reserved() / 1024**2:.2f} MB")

        # Clear cache periodically to avoid fragmentation
        cache_clear_freq = 200
        if batch_idx % cache_clear_freq == 0:
            torch.cuda.empty_cache()

        # Clear GPU memory
        # del preds_G, error_G, acc, iou, prec, recall
        # torch.cuda.empty_cache()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # test per vedere che cazzo succede al tensore in input
        # image_test = resize(
        #         ((in_tensor_dict["image"].cpu().numpy()[0] + 1.0) / 2.0 * 255.0).transpose(1, 2, 0),
        #         (512, 512),
        #         anti_aliasing=True,
        #     )
        # test_path = f"./data/images/{self.cfg.name}/test/prima.png"
        # os.makedirs(os.path.dirname(test_path), exist_ok=True)
        # test_pil = Image.fromarray(image_test.astype('uint8'))
        # test_pil.save(test_path)

        #preds_G, error_G = self.netG(in_tensor_dict)
        #cosi dovrebbe occupare meno memoria
        with torch.no_grad():
            preds_G, error_G = self.netG(in_tensor_dict)
        ...
        #         anti_aliasing=True,
        #     )
        # test_path = f"./data/images/{self.cfg.name}/test/dopo.png"
        # os.makedirs(os.path.dirname(test_path), exist_ok=True)
        # test_pil = Image.fromarray(image_test.astype('uint8'))
        # test_pil.save(test_path)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Clear GPU memory
        del features, inter, sdf, verts_pr, faces_pr
        torch.cuda.empty_cache()
    ...
    def test_epoch_end(self, outputs):
        ...
        #AGGIUNTO PER FARE TEST ANCHE SU THUMAN2
        ...
        np.save(
            osp.join(self.export_dir, f"../{self.cfg.dataset.types[0]}test_results.npy"),
            accu_outputs,
            allow_pickle=True,
        )

        print('accu_outputs: ', accu_outputs)
        print('accu_outputs type: ', type(accu_outputs))

        # Save `accu_outputs` as a text file in `export_dir`
        output_path = osp.join(self.export_dir, f"../{self.cfg.dataset.types[0]}_test_results.txt")
        # Save dictionary as plain text
        with open(output_path, "w") as f:
            for key, value in accu_outputs.items():
                f.write(f"{key}: {value}\n")
    ...
    def render_func(self, in_tensor_dict, dataset="title", idx=0):
        ...
        # test per vedere che cazzo succede al tensore in input
        #self.save_image_test(in_tensor_dict["image"].cpu().numpy()[0], 'in_tensor_image')
        ...
        if sdf is not None:
            ...
            self.save_image_test(image_gt, 'image_gt')
            self.save_image_test(image_pred, 'image_pred')
            for idx, i in enumerate(image_inter):                    
                self.save_image_test(i, f'image_inter_{idx}')

            # Save image to disk 
            #perche' non so come aprire il tensorboard
            image_path = f"./data/images/{self.cfg.name}/Occupancy-{dataset}/{step_id}.png"
            np_path = f"./data/images/{self.cfg.name}/test/'vit_output.npy"
            np.save(np_path, inter[:,0:3,:,:].cpu().numpy())
            # Ensure the directory exists
            os.makedirs(os.path.dirname(image_path), exist_ok=True)
            # Convert the image from NumPy array (H, W, C) to PIL Image and save it
            image_pil = Image.fromarray(image.astype('uint8'))
            image_pil.save(image_path)

            # Clean up GPU memory
            del features, inter, sdf, render, image_pil
            torch.cuda.empty_cache()
